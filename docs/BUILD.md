...
Out comes `dist\UltraNovaCtl-1.2.1-setup.exe`. CI does the same on every push and keeps it
as an artifact.

To exercise it without clicking through the wizard:

```powershell
.\dist\UltraNovaCtl-1.2.1-setup.exe /VERYSILENT /SUPPRESSMSGBOXES /TASKS=autostart
