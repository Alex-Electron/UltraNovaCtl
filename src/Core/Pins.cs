...
public static class Pins
{
    ...
    public sealed class PinInfo
    {
        ...
        public string Flow => DataFlow == DATAFLOW_IN ? "IN  (to synth)"
                            : DataFlow == DATAFLOW_OUT ? "OUT (from synth)"
                            : "?" + DataFlow;
    ...
    public static List<PinInfo> Enumerate(IntPtr filter, out string diagnostic)
    {
        ...
        {
            diagnostic = "CTYPES read failed, error " + err;
            return pins;
        }
        uint count = BitConverter.ToUInt32(ct, 0);
        diagnostic = "pins on filter: " + count;
    ...
    public static void Dump(string filterPath)
    {
        ...
        {
            Console.WriteLine("could not open filter, error " + Marshal.GetLastWin32Error());
            return;
        }
        try
        {
