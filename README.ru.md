...
<img src="img/panel.png" alt="Панель управления UltraNova" width="100%">

*Восемь сенсорных энкодеров над 72-символьным дисплеем, ряд Automap и ручка фильтра, которая
следует за последним тронутым энкодером. В режиме Automap ничего из этого не доходит до
MIDI-порта.*
...
| CC 14-bit, NRPN, RPN | 14-битные параметры |
| Note On/Off | выбор по имени, `Note 042 (F#1)` |
| Pitch Bend | полные 14 бит |
| Aftertouch, Program Change | канальное давление или номер программы |
...
сколько угодно страниц, переключаются кнопками страниц на панели. Значения энкодеров
помнятся по страницам. У назначенных колёс и педалей есть необязательный **pickup**: смена
страницы не сбрасывает параметр в текущую физическую позицию.

**Один экземпляр.** Повторный запуск поднимает уже открытое окно.
...
**Запуск вместе с Windows**, если попросить: галочка в меню трея — и при входе программа
поднимается сразу в трей, окном под руку не лезет.
...
| **UltraNovaCtl** | Инсталлятор или архив из [Releases](https://github.com/Alex-Electron/UltraNovaCtl/releases) — в обоих случаях один самодостаточный `.exe`, так что **.NET ставить не надо**. Инсталлятор кладёт программу в `%LOCALAPPDATA%\Programs`, делает ярлык, умеет включить автозапуск и сам проверяет два компонента выше. Прав администратора не просит. |
...
4. Нажмите **AUTOMAP** на инструменте. В заголовке окна появится `connected`,
...
<img src="img/panel-codes.png" alt="Панель UltraNova со всеми измеренными кодами" width="100%">

Две вещи на этой картинке стоят отдельного взгляда — обе достались дорого.

**Нумерация кнопок и ламп совпадает только до 34.** До этого предела код — это и кнопка, и
её собственная лампа, одного числа достаточно. Дальше нумерации расходятся: 35 — это
`VALUE +`, если нажать, и второй индикатор вокодера, если зажечь. На картинке их различает
цвет чипа: оранжевый — код кнопки, синий — лампы. Нагляднее всего вилка видна в блоке SELECT,
где кнопки 37 и 38 стоят вплотную к лампам 36–41 и не совпадает ни одно число.

**`SYNTH` и `AUTOMAP` нажатий не шлют вообще.** Они меняют режим инструмента, и смена режима —
единственное, что слышит хост. Лампы у них есть, 12 и 14, — и именно поэтому попытка вывести
коды из порядка названий в руководстве даёт неверный ответ для всего, что идёт после них.

Таблица: [docs/PANEL-MAP.md](docs/PANEL-MAP.md), там же нарисованный вариант этой схемы.
...
├── UltraNovaCtl.sln
├── installer/
│   └── UltraNovaCtl.iss       # скрипт Inno Setup
├── docs/
...
│   ├── panel.png              # панель управления крупным планом
│   ├── panel-codes.png        # …она же с проставленными кодами
│   ├── app.png                # окно
│   └── panel-map.svg          # нарисованный вариант схемы
└── src/
...
Самостоятельный редактор звука и менеджер библиотек в одном приложении с Automap,
без необходимости запускать DAW или VST. Обязательное условие — совместная работа
с родным UltraNova Editor, в том числе внутри Ableton.

Требования, порядок реализации и проверки совместимости:
[план редактора и библиотекаря](docs/PRODUCT-PLAN.ru.md).
...
Фотография инструмента: [amazona.de](https://www.amazona.de/). Разворот панели — из
руководства Novation; коды на нём — результат этого проекта.
