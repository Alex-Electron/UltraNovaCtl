...
`VALUE +`, если нажать, и второй индикатор вокодера, если зажечь, — поэтому такие чипы
помечены `B` для кнопки и `L` для лампы. Нагляднее всего это в блоке SELECT, где кнопки
`B37` и `B38` стоят вплотную к лампам `L36`–`L41`, и ни одно число не совпадает.
