...
static partial class Program
{
    static int Main(string[] args)
    {
        ...
        (string name, Action run)[] tests =
        {
            ...
            ("bank select is not a parameter edit", BankSelectIsNotAParameterEdit),
            ("a wide NRPN keeps both halves", AWideNrpnKeepsBothHalves),
            ("attaching forgets the parameter selected before", AttachingForgetsTheParameterSelectedBefore),
            ("probing a lock tells held from absent from unknown", ProbingALockTellsHeldFromAbsentFromUnknown),
            ("an unknown probe is only safe for the informational caller", AnUnknownProbeIsOnlySafeForTheInformationalCaller),
            ("the engine gates writes on the editor's own lock", TheEngineGatesWritesOnTheEditorsOwnLock),
            ("disposal refuses new work afterwards", DisposalRefusesNewWorkAfterwards),
            ("the handshake sends the bytes it is supposed to", TheHandshakeSendsTheBytesItIsSupposedTo),
            ("metadata that is not a metadata message is refused", MetadataThatIsNotAMetadataMessageIsRefused),
            ("a request from an old connection is dropped", ARequestFromAnOldConnectionIsDropped),
            ("the ownership watch notices a takeover while idle", TheOwnershipWatchNoticesATakeoverWhileIdle),
            ("either half of an RPN cancels the NRPN", EitherHalfOfAnRpnCancelsTheNrpn),
            ("opening a file cancels a poll the panel armed", OpeningAFileCancelsAPollThePanelArmed),
            ("every known parameter is coherent", EveryKnownParameterIsCoherent),
            ("the shared filter link bits do not collide", TheSharedFilterLinkBitsDoNotCollide),
            ("no two whole-byte parameters share a byte", NoTwoWholeByteParametersShareAByte),
            ("the screen's cents parameter matches the document", TheScreensCentsParameterMatchesTheDocument),
